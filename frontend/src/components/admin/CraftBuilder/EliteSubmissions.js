        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-black text-slate-900 tracking-tight italic">Form Analysis</h2>
            </div>
          ...
          <div className="relative w-full lg:w-96 group">
            ...
            <input 
              ...
              className="w-full pl-14 pr-8 py-4 bg-white border border-slate-200 rounded-3xl text-[11px] font-black tracking-widest uppercase focus:ring-4 focus:ring-theme-secondary/50 outline-none transition-all shadow-sm"
              value={filter}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-12">
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center opacity-30">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-theme-primary">Synchronizing Registry...</p>
          </div>
        ) : filteredSubmissions.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-300">
            <FiDatabase size={64} className="mb-6 opacity-20" />
            <p className="text-[10px] font-black uppercase tracking-[0.3em] italic text-center">No active data packets detected in this sector.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex flex-col lg:flex-row items-center p-6 lg:px-10 lg:py-8 gap-8">
                  ...
                  <div className="flex items-center gap-5 w-full lg:w-72 flex-shrink-0">
                    <div className="w-14 h-14 rounded-2xl bg-theme-primary flex items-center justify-center text-white font-black text-xl shadow-lg group-hover:scale-110 transition-transform overflow-hidden">
                    ...
                    <div>
                      <h4 className="text-[15px] font-black text-slate-900 tracking-tight">{sub.submitter_name}</h4>
                  ...
                  <div className="flex-1 grid grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Protocol Layer</span>
                      <div className="flex">
                        <span className="bg-theme-secondary text-theme-primary px-3 py-1 rounded-lg text-[10px] font-black border border-theme-secondary uppercase tracking-wider">
                          {sub.form_name}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Access Point</span>
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <button 
                      ...
                      className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${expandedId === sub.id ? 'bg-theme-primary text-white shadow-xl shadow-theme-secondary' : 'bg-slate-50 text-slate-400 hover:bg-theme-secondary hover:text-theme-primary'}`}
                    >
                    ...
                    >
                      <div className="p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 bg-white/50">
                        {Object.entries(sub.data || {}).map(([key, value]) => (
                          <div key={key} className="bg-white p-5 rounded-3xl border border-theme-secondary shadow-sm transition-all hover:bg-theme-secondary/20">
                            <label className="block text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-2">{key}</label>
                      ...
                      <div className="px-10 pb-10">
                        <div className="flex items-center gap-2 p-4 bg-slate-900 rounded-3xl text-theme-primary mb-2">
                           ...
                           <span className="text-[10px] font-black uppercase tracking-widest">API Data Capture Root: /api/v1/{sub.form_slug}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
